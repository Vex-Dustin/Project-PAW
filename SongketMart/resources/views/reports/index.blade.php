                        ...
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @endif
                                    <td>
                                        <span class="d-block fw-bold text-truncate" style="max-width: 200px;">{{ $report->subject }}</span>
                                        <small class="text-muted">{{ $report->created_at->format('d M Y') }}</small>
                                    </td>
                                    <td><span class="badge bg-light text-dark border">{{ $report->type }}</span></td>
                                    <td>
                                        @if($report->status == 'pending')
                                        <span class="badge bg-warning-subtle text-warning px-3 rounded-pill">Menunggu</span>
                                        @elseif($report->status == 'process')
                                        <span class="badge bg-info-subtle text-info px-3 rounded-pill">Diproses</span>
                                        @else
                                    ...
                                    <td class="text-end pe-4">
                                        ...
                                        @endif
                                        <a href="{{ route('reports.show', $report->id) }}" class="btn btn-sm btn-outline-secondary rounded-pill">
                                            Detail
                                        </a>
                                    </td>
                                </tr>
